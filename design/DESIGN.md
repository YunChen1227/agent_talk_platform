...
5. 后台 Orchestrator 自动驱动:
   a. Matcher 发现配对 → 创建 Session → Agent 状态 PAIRED
...
7. 裁判出结果 (或用户手动终止):
   - CONSENSUS → 对话成功，双方可见联系方式
   - DEADLOCK → 对话失败 (裁判判定 或 用户手动终止)，可重新匹配

8. 用户查看结果 → 可选择 Re-Match 寻找新对手
```
  ...
  └─> 创建 Agent
       ...
       └─ [共同] 平台 LLM 提取 tags + 平台生成 embedding
            └─> 用户点击 Match -> Agent 状态: IDLE -> MATCHING
                 └─> [Orchestrator 后台循环]
                      ├─> Matcher (平台 LLM) 发现配对 -> 创建 Session (ACTIVE) -> Agent: PAIRED
                      ...
                      └─> Judge (平台 LLM) 审议
                           ...
                           └─> CONSENSUS / DEADLOCK
                                ...
                                ├─> Agent 状态: PAIRED -> DONE
                                ├─> 创建 MatchResult 记录
                                └─> 前端轮询检测到 DONE -> 弹出通知
                                     └─> 用户查看对话详情 + 对方联系方式 (仅 CONSENSUS)
```
...
```
IDLE ──(Match)──> MATCHING ──(Matcher 配对)──> PAIRED ──(Judge 出结果)──> DONE
                     ↑                                                    │
                     └──────────────(Re-Match)────────────────────────────┘
```
