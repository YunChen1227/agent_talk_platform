...
| **OpenAI SDK (AsyncOpenAI)** | 标签提取、对话回复（Embedding 不经过 OpenAI） |
| **本地 Embedding 服务** | Agent 语义向量：HTTP `POST /v1/embeddings`（默认 Qwen3-Embedding-0.6B，与 `embedding_server.py` 同协议），配置项 `EMBEDDING_API_URL` / `EMBEDDING_DIM` |
...
| **Tags 选择** | 平台 LLM 自动从预置目录提取（用户可手动微调） | 用户从预置目录手动选择，且**可为 Agent 增加用户自定义标签**（见 [DESIGN-PLAZA.md](./DESIGN-PLAZA.md)） |
...
[PAID 用户]
  ...
  → 平台 LLM 从 agent-scope 预置标签目录自动提取 tags (用户可手动微调)
  → 平台生成 embedding
  → Agent 创建完成
...
[FREE 用户]
  ...
  → 用户选择匹配意图偏好 (可选，从"意图"分类一级标签中多选)
  → 用户从 agent-scope 预置标签目录手动选择 tags (必选至少 1 个，可含自定义标签)
...
| `embedding` (存储于 ES/JSON) | Vector[`EMBEDDING_DIM`，默认 1024] | 平台基于描述或 `system_prompt` 调用**本地 Embedding 服务**生成的语义向量，与 Tag 向量维度一致 |
...
Agent 标签与商品标签通过 `tag_category.scope` 彻底隔离，互不交叉：

- **Agent 标签** (`scope = "agent"`)：意图、领域、角色、风格。用户可使用预置标签 + 自定义标签。
- **商品标签** (`scope = "product"`)：商品分类、品质、类型、适用。仅允许平台预置标签。
- **无自动继承**：关联商品时不再将商品标签同步到 Agent 的 `agent_tag`。两套标签体系完全独立，Agent 的匹配画像由 Agent 标签决定，商品属性由商品标签决定。
...
- **LLM 来源分离**: 对话生成的 LLM 调用根据用户 tier 路由；**Embedding 走独立本地模型服务**（非用户 LLM）；标签提取与 Judge 等平台能力仍走平台 LLM。
