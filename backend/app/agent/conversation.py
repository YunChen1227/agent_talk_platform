...
from app.repositories.base import SessionRepository, MessageRepository, AgentRepository, ProductRepository
from app.services.llm import get_random_client
from app.agent.skills import product_salesman
...
async def process_turn(
    ...
    if product_repo and await product_salesman.is_active(current_agent, product_repo):
        salesman_products = await product_salesman.build_product_catalog(current_agent, product_repo)
        if salesman_products:
            system_prompt += product_salesman.build_salesman_prompt(salesman_products)

    response_content = await generate_response(system_prompt, llm_history)

    if salesman_products:
        is_valid, reason = product_salesman.validate_response(response_content, salesman_products)
        if not is_valid:
            retry_prompt = system_prompt + product_salesman.RETRY_REINFORCEMENT
            response_content = await generate_response(retry_prompt, llm_history)
